...
// NewEncryptorFromConfig creates an Encryptor from an env-provided key or an
// auto-generated key persisted at keyPath (matching Yara's app.key pattern).
//
//   - If envValue is non-empty, it is decoded (base64 or hex) and used directly.
//   - Otherwise the file at keyPath is read.  If missing, a new 32-byte key is
//     generated, written to keyPath with 0600 permissions, and used.
...
// Encrypt encrypts plain text with AES-256-GCM.
// Returns a version-prefixed, base64-encoded string.
...
// resolveKey returns a 32-byte key from either envValue or a file at keyPath.
// If neither exists, it generates a new key and writes it to keyPath.
...
// decodeKey tries to interpret value as base64, then hex.
